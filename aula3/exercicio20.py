...
salaries = []
while True:
    salary = float(input("Digite um slaário: "))
    if salary == 0:
        break

    salaries.append(salary)
print(f'{'Salario':>15}{'-':>5}{'Abono':>15}')
for s in salaries:
    bonus = s *0.2
    if bonus <= 100:
        minimus += 1
    ...
    print(f'R$ {s:>15} {'-':>5} R$ { 100 if bonus<=100 else bonus :>15.2f}')

print(f'Foram processados {len(salaries)} colaboradores')
print(f'Total gasto com abonos R$ {totalBonus:.2f}')
print(f'O valor mínimo pago a {minimus} colaboradores')
print(f'O maior valor de abono pago foi de {(max(salaries)*0.2):.2f}')
